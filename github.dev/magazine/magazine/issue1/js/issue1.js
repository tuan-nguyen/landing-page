...
var sampleName = 'issue1',
	samplePath = 'magazine/issue1/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.issue1 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.issue1 .p'+page));
		container.find('.loader').remove();
	});

	img.attr('src', samplePath + 'pages/page_' +  page + '.jpg');
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.issue1').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.issue1').addClass('animated').removeClass('zoom-in');
				}, 0);
...
bookshelf.loadSample(sampleName, function(action) {
	...
	bookshelf.preloadImgs(['FVN_.jpg'], samplePath + 'pages/',
	function() {
