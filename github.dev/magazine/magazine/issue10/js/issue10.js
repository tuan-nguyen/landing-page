...
var sampleName = 'issue10',
	samplePath = 'magazine/issue10/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.issue10 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.issue10 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.issue10').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.issue10').addClass('animated').removeClass('zoom-in');
				}, 0);
