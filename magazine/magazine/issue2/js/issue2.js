...
var sampleName = 'issue2',
	samplePath = 'magazine/issue2/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.issue2 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.issue2 .p'+page));
		container.find('.loader').remove();
	});

	img.attr('src', samplePath + 'pages/FVN_' +  page + '.jpg');
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.issue2').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.issue2').addClass('animated').removeClass('zoom-in');
				}, 0);
...
bookshelf.loadSample(sampleName, function(action) {
	...
	bookshelf.preloadImgs(['FVN_1.jpg'], samplePath + 'pages/',
	function() {
