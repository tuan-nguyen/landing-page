...
var sampleName = 'issue11',
	samplePath = 'magazine/issue11/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.issue11 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.issue11 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.issue11').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.issue11').addClass('animated').removeClass('zoom-in');
				}, 0);
