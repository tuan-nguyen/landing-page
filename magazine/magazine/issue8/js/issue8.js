...
var sampleName = 'issue8',
	samplePath = 'magazine/issue8/';
...
function loadPage(page) {
	...
	var img = $('<img />');
	img.load(function() {
		var container = $('.issue8 .p'+page);
		img.css({width: '100%', height: '100%'});
		img.appendTo($('.issue8 .p'+page));
		...
		when: {
			...
			change: function(event, scale) {
				...
				if (scale==1) {
					...
					$('.issue8').removeClass('animated').addClass('zoom-in');
			...
			zoomOut: function () {
				...
				setTimeout(function(){
					$('.issue8').addClass('animated').removeClass('zoom-in');
				}, 0);
